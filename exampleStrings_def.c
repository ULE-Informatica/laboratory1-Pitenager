...
void gets_example_func(void) {
  ...
  if (fgets(buf, sizeof(buf), stdin) == NULL) {
        fprintf(stderr, "Error en gets_example_func\n");
        exit(1); //return 1;
  }
  buf[strlen(buf) - 1] = '\0';
}
...
const char *get_dirname(const char *pathname) {
  char *slash = (char*)malloc(sizeof(strrchr(pathname, '/')));
...
void get_y_or_n(void) {  
	char response[2]; //char response[8];
...
int main(int argc, char *argv[])
{
  if(argc != 3){
    ...
    //char ptr_char[] = "new string literal";
    //int size_array1 = wcslen(L"аналитик");
    //int size_array2 = 100;

   // char analitic1[size_array1]="аналитик";
   // char analitic2[size_array2]="аналитик";
   //wchar_t analitic3[size_array1]=L"аналитик";

    fputs(get_dirname(__FILE__),stdout);
